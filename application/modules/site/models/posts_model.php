...
class Posts_model extends Model 
{
    ...
    public function getEntries($status = 'published')
	{        
        $query = Database::getInstance()->get(
            "cimp_posts", array('post_status', '=', $status)
        );
	...
	{
		$query = DB::table('cimp_posts')->where('post_slug', $slug)->first();
            // "SELECT * FROM cimp_posts WHERE post_slug = ?", array($slug)
        // );
        return $query;
        if( ! $query->count()) {
            //echo 'No record found';
